...
const state = {
  ...
  currentView: 'landing', // 'landing', 'auth', 'onboarding', 'marketplace', 'dashboard', 'chat', 'internal'
  ...
  // Landing page explorer states
  activeSpecies: 'malaccensis',
    ...
    <aside class="sidebar">
      ...
      <ul class="nav-menu">
        <li class="nav-item ${state.currentView === 'landing' ? 'active' : ''}">
          <a href="#" onclick="window.navTo('landing')">🏠 Home Page</a>
        </li>
...
async function fetchInternalStaffData() {
  ...
  try {
    if (tab === 'moderation-users') {
      ...
      // Fetch real users from DPO users API endpoint
      let dpoUsers = [];
      try {
        dpoUsers = await apiCall('/api/v1/internal/compliance/dpo/users');
      } catch (err) {
            ...
            <tbody>
              ${dpoUsers.map(u => `
                <tr>
                  <td>
                    <strong>${u.email}</strong>
                    <div style="font-size:0.78rem; color:var(--secondary); margin-top:2px;">${u.profile?.business_name || u.profile?.display_name || u.profile?.plantation_name || ''}</div>
                  </td>
                  <td><span class="role-tag" style="background-color:rgba(255,255,255,0.05);">${u.role}</span></td>
                  <td>
                    <span class="trust-badge ${u.id_proof_status === 'approved' ? 'badge-green' : 'badge-gold'}" style="font-size:0.72rem;">
                      ${u.id_proof_status || 'pending'}
                    </span>
                  </td>
                  <td>
                    <span class="trust-badge ${u.email_verified ? 'badge-green' : 'badge-grey'}" style="font-size:0.72rem;">
                      ${u.email_verified ? 'Email Verified' : 'Unverified'}
                    </span>
                  </td>
                  <td>
                    <div style="display:flex; gap:6px; flex-wrap:wrap;">
                      <button class="btn btn-gold" style="padding:6px 12px; font-size:0.8rem;" onclick="window.dpoExportData('${u.id}')">Access Export</button>
                      <button class="btn btn-crimson" style="padding:6px 12px; font-size:0.8rem;" onclick="window.dpoEraseUser('${u.id}')">Erasure Right</button>
                    </div>
                  </td>
                </tr>
              `).join('')}
              ${dpoUsers.length === 0 ? '<tr><td colspan="5" style="text-align:center; color:var(--secondary); padding:20px;">No registered users found.</td></tr>' : ''}
            </tbody>
          </table>
        </div>
      `;
...
// ==========================================================================
// LANDING PAGE GRAPHICS, SIMULATIONS, AND REGULATORY BINDERS
// ==========================================================================

const speciesDatabase = {
  malaccensis: {
    name: "Aquilaria malaccensis",
    common: "Malayan Agarwood / Assam Oudh",
    cites: "Appendix II Protected",
    regions: "Northeast India (Assam), Malaysia, Indonesia, Thailand",
    aroma: "Heavy animalic leather notes, deeply woody, smoky sweetness, long-lasting rich profile.",
    status: "Vulnerable (Wild populations critically depleted)",
    plantation: "Extensively cultivated in Northeast India. High resin yielding wood chips and essential oils."
  },
  crassna: {
    name: "Aquilaria crassna",
    common: "Indochinese Agarwood",
    cites: "Appendix II Protected",
    regions: "Cambodia, Vietnam, Laos, Thailand",
    aroma: "Sweet honeyed undertones, soft medicinal wood, clean warm spice with fruity overtones.",
    status: "Critically Endangered in the wild",
    plantation: "Widely farmed in Indochina. Responds exceptionally well to artificial inoculation services."
  },
  sinensis: {
    name: "Aquilaria sinensis",
    common: "Chinese Agarwood / Hainan Oudh",
    cites: "Appendix II Protected",
    regions: "Southern China, Hainan Island",
    aroma: "Delicate floral sweetness, green tea whispers, smooth and soothing sandalwood-like finish.",
    status: "Vulnerable (Protected under Chinese National Key Conservation list)",
    plantation: "Cultivated extensively in Hainan. Highly sought after for premium incense and traditional medicine."
  },
  gyrinops: {
    name: "Gyrinops ledermannii",
    common: "New Guinea Agarwood",
    cites: "Appendix II Protected",
    regions: "Papua New Guinea, Maluku Islands",
    aroma: "Sharp herbal forest mulch, moist earth, sweet resinous vanilla undertones.",
    status: "Vulnerable due to over-harvesting",
    plantation: "Emerging sector. Requires strict local regulatory clearance before export shipping."
  }
};

function renderLandingView() {
  const activeSpeciesKey = state.activeSpecies || 'malaccensis';
  const species = speciesDatabase[activeSpeciesKey];
  
  // Decide navbar CTA button
  let navCtaBtn = '';
  if (state.user) {
    const targetDash = ['ceo', 'ops_lead', 'moderator', 'support_agent'].includes(state.user.role) ? 'internal' : 'dashboard';
    navCtaBtn = `<button class="btn btn-gold" onclick="window.navTo('${targetDash}')">Go to Dashboard</button>`;
  } else {
    navCtaBtn = `<button class="btn btn-gold" onclick="window.navTo('auth')">Sign In / Register</button>`;
  }

  // Live preview cards
  const previewsHtml = [
    {
      title: "Assam Super King Oud Wood Chips",
      owner: "Royal Assam Oud Plantation",
      type: "Product",
      qty: "15 kg",
    ...
    {
      title: "Organic Cambodi Essential Oil (Grade A)",
      owner: "Phnom Penh Distilleries Ltd",
      type: "Product",
      qty: "5 Liters",
    ...
    {
      title: "High-Yield Inoculation Service package",
      owner: "Bio-Forestry Specialists",
      type: "Service",
      qty: "Up to 500 trees",
      species: "Multi-species adapt",
      price: "Contact for Quote"
    }
  ].map(p => `
    <div class="preview-card-wrapper">
      <div style="display:flex; align-items:center; gap:8px; margin-bottom:12px;">
        <span class="role-tag" style="background-color:var(--accent-green); color:#fff; font-size:0.7rem; padding:2px 6px; border-radius:4px; text-transform:uppercase;">${p.type}</span>
        <h3 style="font-size:1.15rem; font-weight:600; font-family:var(--font-display);">${p.title}</h3>
      </div>
      <p style="color:var(--secondary); font-size:0.85rem; margin-bottom:12px;">Owner: <strong>${p.owner}</strong></p>
      <div style="display:flex; justify-content:space-between; font-size:0.8rem; color:var(--secondary);">
        ...
        <span>🧬 Species: <strong>${p.species}</strong></span>
      </div>
      <div style="margin-top:14px; font-size:0.88rem; color:var(--accent-gold); font-weight:700;">
        Guidance Price: ${p.price}
      </div>
    </div>
  `).join('');
    ...
    <div class="landing-page-wrapper">
      ...
      <nav class="landing-navbar">
        <div class="logo-area">
          ...
          <span class="brand-title">OudhTrade</span>
        </div>
        <div class="nav-links">
          <a href="#features" class="nav-link">Features</a>
          <a href="#species" class="nav-link">Species Explorer</a>
          <a href="#cites-sandbox" class="nav-link">CITES AI Scanner</a>
          <a href="#directory-preview" class="nav-link">Marketplace</a>
      ...
      <section class="landing-hero-section">
        <div>
          <span class="hero-tagline">🌿 Premium Agarwood Trading Registry</span>
          <h1 class="hero-headline">
            Connecting the World's Elite <br>
            <span class="gold-text">Agarwood Trade Partners</span>
          </h1>
          <p class="hero-description">
            OudhTrade is the industry's premier global marketplace directory for Aquilaria wood chips, pure oud essential oils, nursery saplings, and expert inoculation services. Built from the ground up for strict CITES Appendix II environmental compliance.
          </p>
          <div class="hero-actions">
            <button class="btn btn-gold pulse-gold" style="padding:14px 28px; font-size:1rem;" onclick="window.handleExploreDirectory()">Explore Live Registry</button>
            <a href="#cites-sandbox" class="btn" style="padding:14px 28px; font-size:1rem; text-decoration:none;">Try CITES AI Sandbox</a>
          </div>
        </div>
        <div class="hero-img-container">
          <div class="hero-img-card">
            <img src="/premium_oudh_display.png" alt="Premium Oudh Prestige bottle and resinous Agarwood wood chip product photography">
          </div>
          <div class="floating-gold-glow"></div>
        </div>
      </section>
      ...
      <section class="stats-banner">
        <div class="stats-banner-inner">
          <div class="stat-item">
            <div class="stat-num">$14.2M+</div>
            <div class="stat-label">Trade Volume Audited</div>
          </div>
          <div class="stat-item">
            <div class="stat-num">4,200+</div>
            <div class="stat-label">Verified Plantations</div>
          </div>
          <div class="stat-item">
            <div class="stat-num">12+</div>
            <div class="stat-label">Aquilaria Species</div>
          </div>
          <div class="stat-item">
            <div class="stat-num">100%</div>
            <div class="stat-label">CITES Compliant Listings</div>
          </div>
        </div>
      </section>
      ...
      <section id="features" class="landing-section">
        <div class="section-header">
          ...
          <h2 class="section-title">Designed for Global Regulatory Trade</h2>
          <p class="section-subtitle">
            We bridge the gap between strict ecological protection rules and international luxury agarwood demand.
          </p>
        </div>
        
        <div class="features-grid">
          <div class="feature-luxury-card">
            <div class="feature-icon-box">🌍</div>
            <h3 class="feature-card-title">Symmetric Direct Contact</h3>
            <p class="feature-card-desc">
              Any email-verified participant (growers, buyers, nursery owners, specialists) can directly coordinate trade negotiations. Transparent direct networking with no hidden middleman fees.
            </p>
          </div>
          <div class="feature-luxury-card">
            <div class="feature-icon-box">🛡️</div>
            <h3 class="feature-card-title">AI CITES Compliance Auditing</h3>
            <p class="feature-card-desc">
              Every listing is evaluated by our custom NLP CITES engine to prevent wild-harvested smuggling, enforce correct botanical classifications, and request legal nursery certificates.
            </p>
          </div>
          <div class="feature-luxury-card">
            <div class="feature-icon-box">🎖️</div>
            <h3 class="feature-card-title">Double-Layer Identity Verification</h3>
            <p class="feature-card-desc">
              All listings require verified business phone numbers and approved national identity credentials. Build unquestionable trust and credibility across borders.
            </p>
          </div>
        </div>
      </section>

      <!-- CITES NLP Sandbox Evaluator Tool -->
      <section id="cites-sandbox" class="landing-section">
        <div class="section-header">
          <span class="section-tag">Interactive AI Engine</span>
          <h2 class="section-title">CITES Compliance NLP Sandbox</h2>
          <p class="section-subtitle">
            Simulate our automated compliance scan. Enter a mockup listing description to see how the AI flags risks or identifies species.
          </p>
        </div>

        <div class="cites-sandbox-grid">
          <div class="sandbox-form-column">
            <div>
              <h3 style="font-family:var(--font-display); font-size:1.3rem; margin-bottom:20px; color:var(--accent-gold);">Mockup Listing Creator</h3>
              <div class="form-group">
                <label>Listing Title</label>
                <input type="text" id="sandbox-title" class="form-input" placeholder="e.g. Wild harvested old growth Assam wood chips">
              </div>
              <div class="form-grid">
                <div class="form-group">
                  <label>Listing Category</label>
                  <select id="sandbox-type" class="form-select">
                    <option value="product">Oud Wood / Oil Product</option>
                    <option value="plant">Plantation / Tree / Sapling</option>
                    <option value="service">Inoculation Service</option>
                    <option value="by_product">By-product</option>
                  </select>
                </div>
                <div class="form-group">
                  <label>Species Declared</label>
                  <input type="text" id="sandbox-species-suggest" class="form-input" placeholder="e.g. Aquilaria Malaccensis" list="species-list">
                  <datalist id="species-list">
                    <option value="Aquilaria malaccensis">
                    <option value="Aquilaria crassna">
                    <option value="Aquilaria sinensis">
                    <option value="Gyrinops ledermannii">
                  </datalist>
                </div>
              </div>
              <div class="form-group">
                <label>Listing Description</label>
                <textarea id="sandbox-description" class="form-textarea" rows="4" placeholder="Enter listing details. Try typing forbidden keywords like 'wild-harvested', 'natural forest extraction', or 'smuggle' to trigger safety boundary flags..."></textarea>
              </div>
            </div>
            <button class="btn btn-gold" id="btn-run-sandbox" style="width:100%; padding:14px;">Run AI Compliance Scan</button>
          </div>

          <div class="sandbox-results-column" id="sandbox-results-panel">
            <div style="text-align:center; color:var(--secondary); padding:4px;">
              <span style="font-size:3rem; display:block; margin-bottom:16px;">🧬</span>
              <h4 style="font-family:var(--font-display); color:var(--primary); font-size:1.15rem; margin-bottom:8px;">Compliance Engine Ready</h4>
              <p style="font-size:0.9rem; max-width:280px; margin:0 auto; line-height:1.5;">Enter listing parameters on the left and click scan to test safety boundary rules.</p>
            </div>
          </div>
        </div>
      </section>

      <!-- Species Explorer Section -->
      <section id="species" class="landing-section">
        <div class="section-header">
          <span class="section-tag">Botanical Registry</span>
          <h2 class="section-title">CITES Appendix II Species Explorer</h2>
          <p class="section-subtitle">
            Explore the diverse species of the Aquilaria and Gyrinops genera protected under environmental legislation.
          </p>
        </div>

        <div class="species-tab-container">
          <div class="species-nav-list">
            <button class="species-nav-item ${activeSpeciesKey === 'malaccensis' ? 'active' : ''}" onclick="window.setExplorerSpecies('malaccensis')">
              <span>Aquilaria Malaccensis</span> <span>🌿</span>
            </button>
            <button class="species-nav-item ${activeSpeciesKey === 'crassna' ? 'active' : ''}" onclick="window.setExplorerSpecies('crassna')">
              <span>Aquilaria Crassna</span> <span>🌿</span>
            </button>
            <button class="species-nav-item ${activeSpeciesKey === 'sinensis' ? 'active' : ''}" onclick="window.setExplorerSpecies('sinensis')">
              <span>Aquilaria Sinensis</span> <span>🌿</span>
            </button>
            <button class="species-nav-item ${activeSpeciesKey === 'gyrinops' ? 'active' : ''}" onclick="window.setExplorerSpecies('gyrinops')">
              <span>Gyrinops Ledermannii</span> <span>🌿</span>
            </button>
          </div>

          <div class="species-content-card">
            <div>
              <h3 style="font-family:var(--font-display); font-size:1.6rem; color:var(--accent-gold); margin-bottom:6px;">${species.name}</h3>
              <p style="color:var(--secondary); font-size:0.95rem; font-style:italic; margin-bottom:20px;">Commonly known as: <strong>${species.common}</strong></p>
              
              <span class="trust-badge badge-gold" style="font-size:0.75rem;">🛡️ CITES: ${species.cites}</span>
              
              <div class="species-meta-grid">
                <div>
                  <div class="species-meta-label">Primary Region</div>
                  <div class="species-meta-val">${species.regions}</div>
                </div>
                <div>
                  <div class="species-meta-label">Odor/Aromatic Profile</div>
                  <div class="species-meta-val">${species.aroma}</div>
                </div>
                <div>
                  <div class="species-meta-label">Wild Status</div>
                  <div class="species-meta-val">${species.status}</div>
                </div>
                <div>
                  <div class="species-meta-label">Plantation Guidance</div>
                  <div class="species-meta-val">${species.plantation}</div>
                </div>
              </div>
            </div>
            <div style="border-top:1px solid rgba(255,255,255,0.05); padding-top:20px; font-size:0.85rem; color:var(--secondary); display:flex; align-items:center; gap:8px;">
              <span>🚨 <strong>Note:</strong> All export trades require official state CITES certificates.</span>
            </div>
          </div>
        </div>
      </section>

      <!-- Live Directory Preview with Blur overlay -->
      <section id="directory-preview" class="landing-section">
        <div class="section-header">
          <span class="section-tag">Marketplace Sneak-peek</span>
          <h2 class="section-title">Verified Trading Directory</h2>
          <p class="section-subtitle">
            Preview our live listings directory. To unlock detailed quantities, prices, contact channels, and publish listings, create an account.
          </p>
        </div>
        ...
        <div style="position:relative;">
          ...
          <div class="preview-grid-overlay">
            <div class="preview-overlay-box">
              ...
              <h3 style="font-family:var(--font-display); font-size:1.4rem; color:var(--accent-gold); margin-bottom:12px;">Verified Access Registry</h3>
              <p style="color:var(--secondary); font-size:0.95rem; margin-bottom:20px; line-height:1.6;">
                Due to strict Appendix II safety verification requirements, direct trade profiles and contact features are locked to certified registry members.
              </p>
              <div style="display:flex; justify-content:center; gap:12px;">
                <button class="btn btn-gold" onclick="window.navTo('auth')">Sign In / Register</button>
              </div>
            </div>
          </div>
        </div>
      ...
      <section id="faq" class="landing-section">
        <div class="section-header">
          ...
          <p class="section-subtitle">
            Everything you need to know about trading Agarwood on OudhTrade.
          </p>
        </div>

        <div class="faq-grid">
          <div class="faq-card" onclick="window.toggleFaq(0)">
            <div class="faq-question">
              <span>What is OudhTrade and who is it for?</span>
              <span class="faq-toggle-icon">＋</span>
            </div>
            <p class="faq-answer" id="faq-ans-0">
              OudhTrade is a global directory registry connecting certified buyers, sellers, nurseries, farmers, and inoculation specialists dealing in Agarwood and Oud essential oils. It is built to facilitate legally compliant negotiations.
            </p>
          </div>
          
          <div class="faq-card" onclick="window.toggleFaq(1)">
            <div class="faq-question">
              <span>What is CITES Appendix II compliance?</span>
              <span class="faq-toggle-icon">＋</span>
            </div>
            <p class="faq-answer" id="faq-ans-1">
              All species of the genus Aquilaria and Gyrinops (which produce Agarwood) are listed under CITES Appendix II. This means their international trade is regulated to prevent endangerment, requiring exporters to provide government-issued CITES certificates. OudhTrade uses AI to audit listings for compliance and asks sellers to upload documentation.
            </p>
          </div>

          <div class="faq-card" onclick="window.toggleFaq(2)">
            <div class="faq-question">
              <span>How does Symmetric Contact work?</span>
              <span class="faq-toggle-icon">＋</span>
            </div>
            <p class="faq-answer" id="faq-ans-2">
              Symmetric Contact ensures that any email-verified platform member can directly open messaging threads and initiate contact with any other member. Role switching is disabled in OudhTrade to maintain profile credibility (a Buyer cannot switch to a Seller, and vice-versa).
            </p>
          </div>

          <div class="faq-card" onclick="window.toggleFaq(3)">
            <div class="faq-question">
              <span>Are there transaction fees on OudhTrade?</span>
              <span class="faq-toggle-icon">＋</span>
            </div>
            <p class="faq-answer" id="faq-ans-3">
              No. OudhTrade is a listing-only directory platform. We do not process payments or escrow. All transactions, shipping, and CITES permit handling are negotiated directly between the buyers and sellers via our integrated secure messaging system.
            </p>
          </div>
        </div>
      </section>
      ...
      <footer class="luxury-footer">
        <div class="footer-columns">
          <div class="footer-col">
            <h4 style="font-family:var(--font-display); font-size:1.3rem; margin-bottom:15px; color:#fff;">OudhTrade</h4>
            <p style="margin-bottom:20px; font-size:0.88rem;">
              The premier global registry for Agarwood and Aquilaria trade partners. Empowering sustainable forestry under environmental supervision.
            </p>
            <span style="font-size:0.75rem; color:var(--secondary);">© 2026 OudhTrade Global Registry. All rights reserved.</span>
          </div>
          <div class="footer-col">
            <h4>Quick Nav</h4>
            <ul>
              <li><a href="#features">Platform Features</a></li>
              <li><a href="#species">Species Database</a></li>
              <li><a href="#cites-sandbox">AI Compliance Simulator</a></li>
              <li><a href="#directory-preview">Directory Registry</a></li>
            </ul>
          </div>
          <div class="footer-col">
            <h4>Compliance Guidelines</h4>
            <p style="font-size:0.85rem; margin-bottom:12px;">
              Trading wild-harvested Agarwood without local conservation permits violates the international CITES treaty and environmental regulations.
            </p>
            <a href="https://cites.org/eng/app/appendices.php" target="_blank" style="color:var(--accent-gold); font-size:0.85rem; text-decoration:underline;">CITES Appendix II Reference Site</a>
          </div>
        </div>
        <div class="footer-bottom">
          <span>Registered compliance records subject to audit check under statutory forestry control laws.</span>
        </div>
      </footer>
    </div>
  `;
...
function bindLandingEvents() {
  ...
  runBtn.addEventListener('click', () => {
    const title = document.getElementById('sandbox-title').value;
    const desc = document.getElementById('sandbox-description').value;
    const declaredSpecies = document.getElementById('sandbox-species-suggest').value;

    const panel = document.getElementById('sandbox-results-panel');
    if (!panel) return;

    // Toggle loading state
    panel.innerHTML = `
      <div style="display:flex; flex-direction:column; align-items:center; justify-content:center; height:100%; text-align:center; padding:20px;">
        <div style="width:40px; height:40px; border:3px solid rgba(212,175,55,0.2); border-top-color:var(--accent-gold); border-radius:50%; animation: spin 1s linear infinite; margin-bottom:20px;"></div>
        <h4 style="font-family:var(--font-display); font-size:1.1rem; color:var(--primary); margin-bottom:8px;" id="sandbox-load-title">Initializing Scan...</h4>
        <p style="color:var(--secondary); font-size:0.85rem;" id="sandbox-load-detail">Contacting OudhTrade Compliance AI Engine...</p>
      </div>
    `;

    // Inject local spin keyframes style just in case it isn't defined
    if (!document.getElementById('spin-keyframes-style')) {
      const styleEl = document.createElement('style');
      styleEl.id = 'spin-keyframes-style';
      styleEl.innerHTML = `@keyframes spin { to { transform: rotate(360deg); } }`;
      document.head.appendChild(styleEl);
    }

    // Simulated step transitions
    setTimeout(() => {
      const t = document.getElementById('sandbox-load-title');
      const d = document.getElementById('sandbox-load-detail');
      if (t) t.innerText = "Scanning Text Guidelines...";
      if (d) d.innerText = "Parsing listing tags for prohibited CITES keywords...";
    }, 700);

    setTimeout(() => {
      const t = document.getElementById('sandbox-load-title');
      const d = document.getElementById('sandbox-load-detail');
      if (t) t.innerText = "Species Verification...";
      if (d) d.innerText = "Cross-referencing declared species database indices...";
    }, 1400);

    setTimeout(() => {
      const fullText = (title + " " + desc + " " + declaredSpecies).toLowerCase();
      ...
      for (const [kw, explanation] of Object.entries(redFlagsList)) {
        if (fullText.includes(kw)) {
          flags.push(`Red flag keyword '${kw}' detected: ${explanation}`);
          isCompliant = false;
        }
      }
      ...
      for (const s of speciesList) {
        if (fullText.includes(s)) {
          detectedSpecies.push(s.split(' ').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' '));
        }
      }

      if (detectedSpecies.length > 0 && flags.length === 0) {
        flags.push(`Contains protected species: ${detectedSpecies.join(', ')}.`);
      }

      // 3. Compile recommendations
      const recommendations = [];
      if (!isCompliant) {
        recommendations.push("Remove references to unauthorized harvesting or bypass of customs.");
        recommendations.push("Provide a valid CITES registration certificate or cultivation source documentation.");
      } else {
        if (detectedSpecies.length > 0) {
          recommendations.push("Verify CITES Certificate of Origin before publishing listing.");
        } else {
          recommendations.push("Keep records of nursery/plantation cultivation documentation for audits.");
        }
      }

      const score = (92.5 + Math.random() * 7.3).toFixed(2);
      const isAuditSuccessful = isCompliant;
      const badgeClass = isAuditSuccessful ? (detectedSpecies.length > 0 ? 'badge-gold' : 'badge-green') : 'badge-grey';
      const statusLabel = isAuditSuccessful ? (detectedSpecies.length > 0 ? 'Monitoring Required' : 'Passed Basic Scan') : 'Flagged Non-Compliant';
      const borderStyle = isAuditSuccessful ? (detectedSpecies.length > 0 ? 'border-left: 4px solid var(--accent-gold);' : 'border-left: 4px solid var(--accent-green-light);') : 'border-left: 4px solid var(--accent-crimson);';

      panel.innerHTML = `
        <div style="animation: fadeIn 0.4s ease; ${borderStyle} padding:10px 0 10px 16px;">
          <h3 style="font-family:var(--font-display); font-size:1.2rem; color:var(--primary); margin-bottom:10px; display:flex; justify-content:space-between; align-items:center;">
            AI Compliance Scan
            <span class="trust-badge ${badgeClass}" style="font-size:0.75rem;">${statusLabel}</span>
          </h3>
          <p style="color:var(--secondary); font-size:0.85rem; margin-bottom:12px;">Confidence Index: <strong style="color:var(--accent-gold);">${score}%</strong></p>
          
          <div style="margin-bottom:14px; padding:10px; background:rgba(255,255,255,0.02); border-radius:6px; border:1px solid rgba(255,255,255,0.04);">
            <strong style="font-size:0.8rem; display:block; color:var(--secondary); text-transform:uppercase; margin-bottom:4px;">Audit Summary</strong>
            <p style="font-size:0.9rem; line-height:1.4; color:var(--primary);">
              ${isAuditSuccessful 
                ? (detectedSpecies.length > 0 ? `Monitoring Request: Declared CITES Appendix II species detected (${detectedSpecies.join(', ')}). Plantation certificate recommended.` : 'Passed: No prohibited wild-harvest indicators or restricted keywords detected.') 
                : 'Alert: Potential illegal wildlife trade or CITES violation detected due to keywords indicating unpermitted wild harvesting.'}
            </p>
          </div>

          <div style="margin-bottom:14px;">
            <strong style="font-size:0.8rem; display:block; color:var(--secondary); text-transform:uppercase; margin-bottom:4px;">Flags Triggered</strong>
            <ul style="list-style:none; padding-left:0; font-size:0.85rem; line-height:1.4; color:var(--primary);">
              ${flags.length > 0 
                ? flags.map(f => `<li style="margin-bottom:6px;">⚠️ ${f}</li>`).join('') 
                : '<li style="color:var(--secondary);">None</li>'}
            </ul>
          </div>

          <div>
            <strong style="font-size:0.8rem; display:block; color:var(--secondary); text-transform:uppercase; margin-bottom:4px;">Recommended Actions</strong>
            <ul style="padding-left:16px; font-size:0.85rem; line-height:1.4; color:var(--secondary);">
...
window.handleExploreDirectory = () => {
  if (state.user) {
    navigate('marketplace');
  } else {
    showToast('Verified Access Required: Please sign in or register to view directory listings.', true);
    state.authMode = 'login';
    navigate('auth');
  }
};

window.setExplorerSpecies = (key) => {
  state.activeSpecies = key;
  render();
  // Keep species section in view after re-rendering
...
window.toggleFaq = (index) => {
  ...
  if (!ans) return;
  const currentStyle = ans.style.display;
  const isOpen = currentStyle === 'block';
  
  // Close all FAQs first for neat accordion effect
  document.querySelectorAll('.faq-answer').forEach(el => el.style.display = 'none');
  document.querySelectorAll('.faq-toggle-icon').forEach(el => el.innerText = '＋');

  if (!isOpen) {
    ans.style.display = 'block';
    const card = ans.closest('.faq-card');
    if (card) {
