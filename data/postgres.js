...
const seedDatabase = (db) => {
  // If database contains old Mettl-AI seeds (e.g. hr@stripe.com) or lacks OudhTrade CEO, re-seed
  const needsReseed = !db.users || 
                      db.users.length === 0 || 
                      !db.users.some(u => u.email === "ceo@oudhtrade.com") ||
                      db.users.some(u => u.email === "hr@stripe.com") ||
                      !db.users.some(u => u.password === "password123");
